...
agg_newdata <- pred %>%
  ...
  ungroup() %>%
  group_by(year, week) %>%
  summarise(pp = sum(pp)) # això és sumant els counts diaris setmanalment
...
temporal_variability_plot <- ggplot(data = agg_newdata, aes(x = week, y = pp, color = year, group = year)) +
  ...
  scale_y_continuous(breaks = c(0, 6, 12, 18, 24)) + # Set breaks for the Y-axis
...
aedes_albopictus_df_mean <- aedes_albopictus_df %>%
...
aedes_albopictus_df_mean$week <- as.integer(aedes_albopictus_df_mean$week)
...
scale_factor <- max(agg_newdata_mean$pp) / max(aedes_albopictus_df_mean$females)

# Define manual breaks for the y-axis
y_breaks_pred <- c(0, 1, 2, 3)
y_breaks_real <- c(0, 2.9, 5.8, 8.7)
...
observed_predicted_plot <- ggplot(data = agg_newdata_mean, aes(x = week)) +
  ...
  geom_line(data = aedes_albopictus_df_mean, aes(x = week, y = females * scale_factor, color = "Observed"), 
            size = 1) +
